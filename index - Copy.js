...
function calculate_subscription(expiry_date, months_to_buy, monthly_cost, annual_cost) {
    ...
    if (months_to_buy == 12) {
        ...
        if (next1.diff(next15, 'days') < 0) {
            ...
            // console.log("(" + moment(next1).format("DD/MM/YYYY") + ',' + (annual_cost - nextyear.diff(next1, 'days') * (annual_cost / 365)) + ")");
            arr[0] = moment(next1).format("DD/MM/YYYY");
            arr[1] = (annual_cost - nextyear.diff(next1, 'days') * (annual_cost / 365));
        }
        else {

            // console.log("(" + moment(next15).format("DD/MM/YYYY") + ',' + (annual_cost - nextyear.diff(next15, 'days') * (annual_cost / 365)) + ")");
        ...
        if (next1.diff(next15, 'days') > 0) {
            ...
            if (moment(nextdate).format("DD") == 1) {
                //console.log("(" + moment(next1).format("DD/MM/YYYY") + ',' + (monthly_cost * months_to_buy) + ")");
                arr[0] = moment(next1).format("DD/MM/YYYY");
                arr[1] = (monthly_cost * months_to_buy);
            }
            else {
                //console.log("(" + moment(next1).format("DD/MM/YYYY") + ',' + (monthly_cost * ((months_to_buy - 1)) + (30 - moment(nextdate).format("DD") + 1) * (monthly_cost / 30)) + ")");
                //   console.log("(" + moment(next1).format("DD/MM/YYYY") + ',' + (monthly_cost * (months_to_buy - 1) + nextdate.add(-1, 'months').diff(next1, 'days') * (monthly_cost / 30)) + ")");
                arr[0] = moment(next1).format("DD/MM/YYYY");
            ...
            if (moment(nextdate).format("DD") == 15) {
                //console.log("(" + moment(next15).format("DD/MM/YYYY") + ',' + (monthly_cost * months_to_buy) + ")");
                arr[0] = moment(next15).format("DD/MM/YYYY");
                arr[1] = (monthly_cost * months_to_buy);
            }
            else {
                //  console.log("(" + moment(next15).format("DD/MM/YYYY") + ',' + (monthly_cost * ((months_to_buy - 1)) - nextdate.add(-1, 'months').diff(next15, 'days') * (monthly_cost / 30)) + ")");
                // console.log("(" + moment(next15).format("DD/MM/YYYY") + ',' + (monthly_cost * ((months_to_buy - 1)) + (30 - moment(nextdate).format("DD") + 15) * (monthly_cost / 30)) + ")");
                arr[0] = moment(next15).format("DD/MM/YYYY");
...
var all = calculate_subscription("1/06/2018", 12, 1000, 8000)
